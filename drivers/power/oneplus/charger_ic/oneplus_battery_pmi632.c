...
#include "../gauge_ic/oneplus_bq27541.h"

#include "linux/oem/project_info.h"

#define ONEPLUS_CHG_MONITOR_INTERVAL round_jiffies_relative(msecs_to_jiffies(5000))

static int __debug_mask = 0xff & (~PR_REGISTER) & (~PR_WLS);
static struct oneplus_chg_chip *g_oneplus_chip = NULL;
...
static int onepluschg_charge_enble(void);
static int onepluschg_charge_disable(void);
static int onepluschg_suspend(void);
static int onepluschg_unsuspend(void);
static void onepluschg_set_chargerid_switch(int value);
static int onepluschg_get_chargerid_switch(void);
static bool onepluschg_is_usb_present(void);
static void onepluschg_rerun_aicl(void);
static void onepluschg_set_usb_props_type(enum power_supply_type type);
static bool onepluschg_get_otg_switch(void);
static bool onepluschg_get_otg_online(void);
static void onepluschg_set_otg_switch_ui(bool value);
static void onepluschg_monitor_func(struct work_struct *work);

static void typec_disable_cmd_work(struct work_struct *work);
static int onepluschg_read_adc(int channel);
static int onepluschg_set_fcc(int current_ma);
static int onepluschg_get_charge_enable(void);
static int onepluschg_set_vbus(int vbus);
...
static void op_disconnect_vbus(struct smb_charger *chg, bool enable);
...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	if (chg->pd_active) {
		...
		if (!(apsd_result->pst == POWER_SUPPLY_TYPE_USB_FLOAT &&
			...
			onepluschg_set_usb_props_type(apsd_result->pst);
		}
	}
...
int smblib_vconn_regulator_enable(struct regulator_dev *rdev)
{
	...
	/* VCONN orientation is oneplussite to that of CC */
	orientation =
...
int smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	struct smb_charger *chg = rdev_get_drvdata(rdev);
	int rc;
	struct oneplus_chg_chip *chip = g_oneplus_chip;
...
static void onepluschg_set_otg_switch_ui(bool value)
{
	...
	if (!g_oneplus_chip) {
		chg_err("fail to init oneplus_chip\n");
		return;
	}


	chg = &g_oneplus_chip->pmic_spmi.smb5_chip->chg;
	...
	if (vbus_rising == true) {
		chg_debug("vbus_rising, stat 0x1544 = 0x%x, otg_switch = %d, otg_online = %d, ui_otg_switch = %d\n",
			...
			stat, g_oneplus_chip->otg_switch, g_oneplus_chip->otg_online, g_oneplus_chip->ui_otg_switch);
		return;
	}

	g_oneplus_chip->otg_online = value;

	if (g_oneplus_chip->otg_online) {
		chg_debug("oneplus detected gpio low\n");
		...
		rc0 = smblib_request_dpdm(chg, true);
		if (rc0 < 0)
			smblib_err(chg, "Couldn't to enable DPDM rc0=%d\n", rc0);
	} else {
		chg_debug("oneplus detected gpio high\n");
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
			TYPEC_POWER_ROLE_CMD_MASK | TYPEC_TRY_MODE_MASK, EN_SNK_ONLY_BIT);
		if (rc < 0)
			chg_err("fail to write pmic register\n");

		rc0 = smblib_request_dpdm(chg, false);
		if (rc0 < 0)
			smblib_err(chg, "Couldn't to enable DPDM rc0=%d\n", rc0);
	}
...
void onepluschg_set_otg_switch(bool value)
{
	int rc;
	int rc0;
	//int rc1;
...
EXPORT_SYMBOL(onepluschg_set_otg_switch);
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (chg->input_suspend_locked) {
			chg->input_suspend_locked = false;
			val.intval = 0;
			smblib_set_prop_input_suspend(chg, &val);
		}

		oneplus_warp_reset_fastchg_after_usbout();
		if (oneplus_warp_get_fastchg_started() == false && g_oneplus_chip) {
			onepluschg_set_chargerid_switch(0);
			g_oneplus_chip->chargerid_volt = 0;
			g_oneplus_chip->chargerid_volt_got = false;
			g_oneplus_chip->charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
			oneplus_chg_wake_update_work();
		}
	...
	if (vbus_rising) {
		cancel_delayed_work_sync(&chg->onepluschg_monitor_work);
		schedule_delayed_work(&chg->onepluschg_monitor_work, ONEPLUS_CHG_MONITOR_INTERVAL);
//		oneplus_force_to_fulldump(true);
	} else {
		#if 0
		if (g_oneplus_chip && g_oneplus_chip->ui_otg_switch != g_oneplus_chip->otg_switch)
			onepluschg_set_otg_switch(g_oneplus_chip->ui_otg_switch);
		#endif
		cancel_delayed_work_sync(&chg->onepluschg_monitor_work);
	}
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->onepluschg_monitor_work, onepluschg_monitor_func);
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	rc = of_property_read_u32(node, "op,third-protect-loop-temp", &chg->third_protect_loop_temp);
	if (rc < 0)
		chg->third_protect_loop_temp = THIRD_PROTECT_LOOP_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "third_protect_loop_temp=%d\n", chg->third_protect_loop_temp);
	rc = of_property_read_u32(node, "op,third-protect-interval-temp", &chg->third_protect_interval_temp);
	if (rc < 0)
		chg->third_protect_interval_temp = THIRD_PROTECT_INTERVAL_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "third_protect_interval_temp=%d\n", chg->third_protect_interval_temp);
	rc = of_property_read_u32(node, "op,fast-check-threshold-temp", &chg->fast_check_threshold_temp);
	if (rc < 0)
		chg->fast_check_threshold_temp = FAST_CHECK_THRESHOLD_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "fast_check_threshold_temp=%d\n", chg->fast_check_threshold_temp);
	rc = of_property_read_u32(node, "op,high-temp-short-check-timeout", &chg->high_temp_short_check_timeout);
	if (rc < 0)
		chg->high_temp_short_check_timeout = HIGH_TEMP_SHORT_CHECK_TIMEOUT;
	smblib_dbg(chg, PR_USBTEMP, "high_temp_short_check_timeout=%d\n", chg->high_temp_short_check_timeout);
	rc = of_property_read_u32(node, "op,fast-check-interval-period", &chg->fast_check_interval_period);
	if (rc < 0)
		chg->fast_check_interval_period = FAST_CHECK_INTERVAL_PERIOD;
	smblib_dbg(chg, PR_USBTEMP, "fast_check_interval_period=%d\n", chg->fast_check_interval_period);
	rc = of_property_read_u32(node, "op,normal-check-interval-period", &chg->normal_check_interval_period);
	if (rc < 0)
		chg->normal_check_interval_period = NORMAL_CHECK_INTERVAL_PERIOD;
	smblib_dbg(chg, PR_USBTEMP, "normal_check_interval_period=%d\n", chg->normal_check_interval_period);
	rc = of_property_read_u32(node, "op,first-protect-connector-temp", &chg->first_protect_connector_temp);
	if (rc < 0)
		chg->first_protect_connector_temp = FIRST_PROTECT_CONNECTOR_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "first_protect_connector_temp=%d\n", chg->first_protect_connector_temp);
	rc = of_property_read_u32(node, "op,second-protect-interval-temp", &chg->second_protect_interval_temp);
	if (rc < 0)
		chg->second_protect_interval_temp = SECOND_PROTECT_INTERVAL_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "second_protect_interval_temp=%d\n", chg->second_protect_interval_temp);
	rc = of_property_read_u32(node, "op,second-protect-connector-temp", &chg->second_protect_connector_temp);
	if (rc < 0)
		chg->second_protect_connector_temp = SECOND_PROTECT_CONNECTOR_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "second_protect_connector_temp=%d\n", chg->second_protect_connector_temp);
	rc = of_property_read_u32(node, "op,third-protect-base-temp", &chg->third_protect_base_temp);
	if (rc < 0)
		chg->third_protect_base_temp = THIRD_PROTECT_BASE_TEMP;
	smblib_dbg(chg, PR_USBTEMP, "third_protect_base_temp=%d\n", chg->third_protect_base_temp);
	rc = of_property_read_u32(node, "op,third-protect-rise-rate", &chg->third_protect_rise_rate);
	if (rc < 0)
		chg->third_protect_rise_rate = THIRD_PROTECT_RISE_RATE;
	smblib_dbg(chg, PR_USBTEMP, "third_protect_rise_rate=%d\n", chg->third_protect_rise_rate);
...
static int op_get_usb_connector_temp(int which)
{
	int i, resistance, temperature, voltage = 0;

	voltage = onepluschg_read_adc(which);
	resistance = voltage_convert_resistance(voltage, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_100KOHM);
	resistance_convert_temperature(resistance, temperature, i, ntc_table_100K);
	return temperature;
}
...
static int oneplus_chg_get_charger_subtype(void);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		val->intval = onepluschg_get_otg_switch();
		break;
	case POWER_SUPPLY_PROP_OTG_ONLINE:
		val->intval = onepluschg_get_otg_online();
		break;
	...
	case POWER_SUPPLY_PROP_USBTEMP_VOLT_L:
		if (g_oneplus_chip)
			val->intval = g_oneplus_chip->usbtemp_volt_l;
		else
			val->intval = -ENODATA;
		break;
	case POWER_SUPPLY_PROP_USBTEMP_VOLT_R:
		if (g_oneplus_chip)
			val->intval = g_oneplus_chip->usbtemp_volt_r;
		else
			val->intval = -ENODATA;
		break;
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		val->intval = oneplus_chg_get_charger_subtype();
		break;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ADAPTER_CC_MODE:
		...
		break;
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		//onepluschg_set_otg_switch(!!val->intval);
		onepluschg_set_otg_switch_ui(!!val->intval);
		break;
	case POWER_SUPPLY_PROP_APSD_RERUN:
		del_timer_sync(&chg->apsd_timer);
		chg->apsd_ext_timeout = false;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FLASH_ACTIVE:
		if ((chg->chg_param.smb_version == PMI632_SUBTYPE)
			...
			smblib_set_opt_switcher_freq(chg, chg->flash_active ?
				chg->chg_freq.freq_removal : chg->chg_freq.freq_5V);
	...
	union power_supply_propval *val)
{
    ...
    rc = oneplus_ac_get_property(psy, psp, val);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		rc = smblib_get_prop_input_current_settled(chg, val);
		break;
	case POWER_SUPPLY_PROP_USB_CONNECTOR_TEMP1:
		val->intval = op_get_usb_connector_temp(USB_TEMPERATURE1);
		break;
	case POWER_SUPPLY_PROP_USB_CONNECTOR_TEMP2:
		val->intval = op_get_usb_connector_temp(USB_TEMPERATURE2);
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		val->intval = onepluschg_get_charge_enable();
		break;
	...
	case POWER_SUPPLY_PROP_CONNECT_DISABLE:
		...
		break;
	default:
		rc = oneplus_battery_get_property(psy, psp, val);
	}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		chg->input_suspend_locked = val->intval;
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		if (val->intval == 1)
			onepluschg_charge_enble();
		else if (val->intval == 0)
			onepluschg_charge_disable();
		break;
	case POWER_SUPPLY_PROP_OP_DISABLE_CHARGE:
		if (val->intval == 1)
			onepluschg_charge_disable();
	...
	case POWER_SUPPLY_PROP_CONNECT_DISABLE:
		...
		break;
	default:
		rc = oneplus_battery_set_property(psy, prop, val);
	}
		...
		enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_TORCH_CURRENT_WA:
		return 1;
	default:
		rc = oneplus_battery_property_is_writeable(psy, psp);
		break;
	}
...
#ifdef CONFIG_HIGH_TEMP_VERSION
	/* when 5p1k BAT_THERMAL resistance  */
	smblib_write(chg, 0x1094, 0x13);
	smblib_write(chg, 0x1095, 0xBF);
	smblib_write(chg, 0x1096, 0x5E);
	smblib_write(chg, 0x1097, 0x68);
	smblib_write(chg, 0x1098, 0x13);
	smblib_write(chg, 0x1099, 0x63);
	smblib_write(chg, 0x109A, 0x5A);
	smblib_write(chg, 0x109B, 0x12);
#endif
...
static int smb5_show_charger_status(struct smb5 *chip)
{
	...
	if (g_oneplus_chip)
		batt_health = oneplus_chg_get_prop_batt_health(g_oneplus_chip);
...
static struct delayed_work onepluschg_debug_work;
...
static void onepluschg_debug_func(struct work_struct *w)
{
	int i, j, k;
	u8 stat[16];
	struct smb_charger *chg = NULL;
	...
	schedule_delayed_work(&onepluschg_debug_work, msecs_to_jiffies(charge_debug_delay > 1000 ? charge_debug_delay : 1000));
}

bool onepluschg_pd_sdp = false;
static int onepluschg_get_charger_type(void)
{
	u8 apsd_stat;
	int rc;
	struct smb_charger *chg = NULL;
	...
	if (g_oneplus_chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN)
		chg->pre_current_ma = -1;
	...
	if (!(apsd_stat & APSD_DTC_STATUS_DONE_BIT)) {
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_PD) {
			if (onepluschg_pd_sdp == true) {
				type = POWER_SUPPLY_TYPE_USB;
			} else {
				type = POWER_SUPPLY_TYPE_USB_DCP;
			}
			onepluschg_pd_sdp = false;
			goto get_type_done;
		}
		type = POWER_SUPPLY_TYPE_UNKNOWN;
		goto get_type_done;
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_PD) {
		if (strcmp("SDP", apsd_result->name) == 0 || onepluschg_pd_sdp == true) {
			type = POWER_SUPPLY_TYPE_USB;
		} else {
			type = POWER_SUPPLY_TYPE_USB_DCP;
		}
...
static int onepluschg_get_fv_monitor(struct oneplus_chg_chip *chip)
{
    int default_fv = 0;
...
#ifdef CONFIG_ONEPLUS_SHORT_C_BATT_CHECK
    if (oneplus_short_c_batt_is_prohibit_chg(chip) && default_fv > chip->limits.short_c_bat_vfloat_mv)
        default_fv = chip->limits.short_c_bat_vfloat_mv;
#endif
    return default_fv;
}

static int onepluschg_get_vbatt_full_vol_sw(struct oneplus_chg_chip *chip)
{
	int default_fv = 0;
...
#ifdef CONFIG_ONEPLUS_SHORT_C_BATT_CHECK
	if (oneplus_short_c_batt_is_prohibit_chg(chip) && default_fv > chip->limits.short_c_bat_vfloat_sw_limit)
...
static void onepluschg_monitor_func(struct work_struct *work)
{
	...
	//if (boot_mode == MSM_BOOT_MODE_RF || boot_mode == MSM_BOOT_MODE_WLAN)//debug for bring up
		//return;
	...
	if (oneplus_warp_get_fastchg_started() == true || chip->charger_volt < 4400) {
		counts = 0;
		goto rerun_work;
	}
	if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP)
		rechg_vol = onepluschg_get_fv_monitor(chip) - 300;
	else if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP)
		rechg_vol = onepluschg_get_fv_monitor(chip) - 200;
	else
		rechg_vol = onepluschg_get_fv_monitor(chip) - 100;
	if ((chip->batt_volt > rechg_vol - 10) && chip->batt_full) {
		counts = 0;
		goto rerun_work;
	} else if (chip->batt_volt > onepluschg_get_vbatt_full_vol_sw(chip) - 10) {
		counts = 0;
		goto rerun_work;
	}
	...
	if (counts >= (chip->batt_full ? 8 : 3)) {//because rechg counts=6
		...
		if (rc < 0) {
			printk(KERN_ERR "onepluschg_monitor_func: Couldn't get BATTERY_CHARGER_STATUS_8_REG status rc=%d\n", rc);
			goto rerun_work;
		}
		if (get_client_vote(chg->usb_icl_votable, BOOST_BACK_VOTER) == 0
				&& get_effective_result(chg->usb_icl_votable) <= USBIN_25MA) {
			printk(KERN_ERR "onepluschg_monitor_func: boost back\n");
			if (chg->wa_flags & BOOST_BACK_WA)
				vote(chg->usb_icl_votable, BOOST_BACK_VOTER, false, 0);
		}
		...
		if (stat & PRE_TERM_BIT) {
			...
			printk(KERN_ERR "onepluschg_monitor_func: PRE_TERM_BIT is set[0x%x], clear it\n", stat);
			rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 1);
			if (rc < 0) {
				printk(KERN_ERR "onepluschg_monitor_func: Couldn't set USBIN_SUSPEND_BIT rc=%d\n", rc);
				goto rerun_work;
			}
			msleep(50);
			rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 0);
			if (rc < 0) {
				printk(KERN_ERR "onepluschg_monitor_func: Couldn't clear USBIN_SUSPEND_BIT rc=%d\n", rc);
				goto rerun_work;
			}
			msleep(10);
			rc = smblib_masked_write(chg, AICL_CMD_REG, RESTART_AICL_BIT, RESTART_AICL_BIT);
			if (rc < 0) {
				printk(KERN_ERR "onepluschg_monitor_func: Couldn't set RESTART_AICL_BIT rc=%d\n", rc);
				goto rerun_work;
			}
			printk(KERN_ERR "onepluschg_monitor_func: ichg[%d], fv[%d]\n", chip->icharging, onepluschg_get_fv_monitor(chip));
		}
		counts = 0;
	}
...
{
	int temp = 0, step_value = 0, temp_1 = 0, temp_2 = 0;

	temp_1 = op_get_usb_connector_temp(USB_TEMPERATURE1)/10;
	temp_2 = op_get_usb_connector_temp(USB_TEMPERATURE2)/10;
...
static void op_usbtemp_dischg(void)
{
	int rc;
	struct smb_charger *chg = NULL;

	if (!g_oneplus_chip) {
		chg_err("fail to init oneplus_chip or adc_pinctrl\n");
		return;
	}
	...
	if (g_oneplus_chip->otg_switch == true) {
	...
	g_oneplus_chip->chg_ops->charging_disable();
	usleep_range(10000,11000);
	g_oneplus_chip->chg_ops->charger_suspend();
	usleep_range(10000,11000);
	...
	mutex_lock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl, g_oneplus_chip->normalchg_gpio.dischg_enable);
	mutex_unlock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
}
...
static void op_disconnect_vbus(struct smb_charger *chg, bool enable)
{
	...
	if (!enable) {
		mutex_lock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl, g_oneplus_chip->normalchg_gpio.dischg_disable);
		mutex_unlock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		usleep_range(10000, 11000);
		g_oneplus_chip->chg_ops->charger_unsuspend();
		usleep_range(10000, 11000);
		g_oneplus_chip->chg_ops->charging_enable();
...
static void op_connecter_recovery_charge_work(struct work_struct *work)
{
	...
	if (!chg->disconnect_vbus || boot_mode == MSM_BOOT_MODE_AGING)
		return;

	chg->connecter_temp = get_usb_temp(chg);
	batt_temp = oneplus_gauge_get_batt_temperature()/10;
	interval_temp = chg->connecter_temp - batt_temp;

	pr_info("disconnect_vbus=%d, connecter_temp=%d, interval_temp=%d",
			chg->disconnect_vbus, chg->connecter_temp, interval_temp);

	if ((chg->connecter_temp <= 50) && (interval_temp <= 10)) {
...
static void op_connecter_temp_check_work(struct work_struct *work)
{
	...
	if (!chg->vbus_present || boot_mode == MSM_BOOT_MODE_AGING)
		return;
	...
	if (isQcTorchEn && !isVbusSet) {
		onepluschg_set_vbus(VBUS_LIMIT_5V);
		isVbusSet = true;
		}
	else if (!isQcTorchEn && isVbusSet) {
		onepluschg_set_vbus(g_oneplus_chip->limits.qc_input_volt_charger_v);
		isVbusSet = false;
		}
	...
	pr_debug("connecter_temp:%d,batt_temp:%d,interval_temp:%d,count_total:%d,count_run:%d,connector_short:%d\n",
			chg->connecter_temp,
			batt_temp,
			interval_temp,
			chg->count_total,
	...
	if (chg->connecter_temp >= chg->first_protect_connector_temp) { /* >=60 */
	...
	if (chg->connecter_temp < chg->fast_check_threshold_temp)
		...
		&& (chg->connecter_temp < chg->first_protect_connector_temp))
		/*time need optimize depend on test*/
		schedule_delayed_work(&chg->connecter_check_work,
			msecs_to_jiffies(chg->fast_check_interval_period));
	else
		pr_info("connecter_temp:%d\n", chg->connecter_temp);
}

static int onepluschg_parse_custom_dt(struct oneplus_chg_chip *chip)
{
...
static int onepluschg_get_chargerid(void)
{
	int chgid;
	...
	if (IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.chargerid_adc_default)) {
		chg_debug("chargerid not support return 0\n");
		return 0;
	}

	if ((boot_with_console() == false) && (!IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.chargerid_adc_default))) {
		mutex_lock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl, g_oneplus_chip->normalchg_gpio.uart_bias_disable);
		mutex_unlock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		msleep(10);
	}

	chgid = onepluschg_read_adc(CHARGERID_VOLTAGE);

	if ((boot_with_console() == false) && (!IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.chargerid_adc_default))) {
		mutex_lock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl, g_oneplus_chip->normalchg_gpio.uart_pull_down);
		mutex_unlock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	}
...
static int onepluschg_get_chargerid_switch(void)
{
	...
	if (g_oneplus_chip->normalchg_gpio.chargerid_switch_gpio <= 0) {
		chg_err("chargerid_switch_gpio not exist, return\n");
		return 0;
	}

	return gpio_get_value(g_oneplus_chip->normalchg_gpio.chargerid_switch_gpio);
}
...
static void onepluschg_set_chargerid_switch(int value)
{
	...
	if (IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.pinctrl)
		|| IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.chargerid_switch_active)
		|| IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.chargerid_switch_sleep)
		|| IS_ERR_OR_NULL(g_oneplus_chip->normalchg_gpio.chargerid_switch_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}

	if (oneplus_warp_get_adapter_update_real_status() == ADAPTER_FW_NEED_UPDATE
	...
	if (value)
		pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl,
			g_oneplus_chip->normalchg_gpio.chargerid_switch_active);
	else
		pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl,
			g_oneplus_chip->normalchg_gpio.chargerid_switch_sleep);

	mutex_unlock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);

	chg_debug("set usb_switch_1 = %d, result = %d\n", value, onepluschg_get_chargerid_switch());
}
...
static bool onepluschg_get_otg_switch(void)
{
	...
	return g_oneplus_chip->otg_switch;
}
...
static bool onepluschg_get_otg_online(void)
{
	...
	if (chg->typec_mode >= POWER_SUPPLY_TYPEC_SINK && chg->typec_mode <= POWER_SUPPLY_TYPEC_POWERED_CABLE_ONLY)
		g_oneplus_chip->otg_online = true;
	else
		g_oneplus_chip->otg_online = false;
	return g_oneplus_chip->otg_online;
}
...
static void onepluschg_dump_regs(void)
{
	/* do nothing */
}

static int onepluschg_kick_wdt(void)
{
	return 0;
}

static int onepluschg_hw_init(void)
{
	enum oem_boot_mode boot_mode = get_boot_mode();
	if (boot_mode != MSM_BOOT_MODE_RF)
		onepluschg_unsuspend();
	else
		onepluschg_suspend();
...
static int onepluschg_set_vbus(int vbus)
{
	...
	rc = power_supply_set_property(chip->pmic_spmi.smb5_chip->chg.usb_main_psy,
			 ...
			 &ret);
	if (rc < 0) {
		pr_err("onepluschg_set_vbus failed rc = %d\n",rc);
		return rc;
	}else{
		pr_debug("onepluschg_set_vbus limit = %d",vbus);
	}
...
static int onepluschg_set_fcc(int current_ma)
{
	...
	chg_debug("onepluschg_set_fcc = %d\n",current_ma);
...
static void onepluschg_set_aicl_point(int vol)
{
	/* do nothing */
}

static void onepluschg_aicl_enable(bool enable)
{
	int rc = 0;
	u8 aicl_op;
	struct oneplus_chg_chip *chip = g_oneplus_chip;
...
static void onepluschg_rerun_aicl(void)
{
	onepluschg_aicl_enable(false);
	msleep(50);
	onepluschg_aicl_enable(true);
}

static bool onepluschg_is_normal_mode(void)
{
	return true;
}

static bool onepluschg_is_suspend_status(void)
{
	int rc = 0;
	u8 stat;
	struct smb_charger *chg = NULL;
	...
	rc = smblib_read(chg, POWER_PATH_STATUS_REG, &stat);
	if (rc < 0) {
		chg_err("oneplus_chg_is_suspend_status: Couldn't read POWER_PATH_STATUS rc = %d\n", rc);
		return false;
	}
...
static void onepluschg_clear_suspend(void)
{
	int rc;
	struct smb_charger *chg = NULL;
	...
	rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 1);
	if (rc < 0)
		chg_err("oneplus_chg_monitor_work: Couldn't set USBIN_SUSPEND_BIT rc = %d\n", rc);
	msleep(50);

	rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 0);
	if (rc < 0)
		chg_err("oneplus_chg_monitor_work: Couldn't clear USBIN_SUSPEND_BIT rc = %d\n", rc);
}

static void onepluschg_check_clear_suspend(void)
{
	use_present_status = true;
	onepluschg_clear_suspend();
	use_present_status = false;
}

static void onepluschg_usbin_collapse_irq_enable(bool enable)
{
...
struct oneplus_qc_config {
	...
	int jeita;
	int state;
	int (*handler)(struct oneplus_qc_config *config, void *data);
};

struct oneplus_qc_config_param {
	int temp;
	int volt;
};

static int oneplus_qc_input_current_hanlder(struct oneplus_qc_config *config, void *data);

static struct oneplus_qc_config oneplus_qc_config_data[] = {
	{
		.qc_hv_icl = 364,
		.qc_lv_icl = 364,
		.qc_ledon_icl = 2000,
		...
		.state = -ENOKEY,
		.handler = oneplus_qc_input_current_hanlder,
	},
	{
		.qc_hv_icl = 1900,
		.qc_lv_icl = 1140,
	...
	},
	{
		...
		.state = -ENOKEY,
		.handler = oneplus_qc_input_current_hanlder,
		//.handler = oneplus_qc_ht_fv_handler,
	},
};

static int oneplus_qc_check_icl(struct oneplus_chg_chip *chip) {
	int size = sizeof(oneplus_qc_config_data) / sizeof(struct oneplus_qc_config);
	int i;
	struct oneplus_qc_config_param param;
	...
	for (i = 0; i < size; i++) {
		if (oneplus_qc_config_data[i].handler) {
			oneplus_qc_config_data[i].handler(&oneplus_qc_config_data[i], (void*)&param);
		}
	}
	return 0;
}

static int oneplus_qc_set_icl(int current_ma) {
	int rc = 0;
	struct oneplus_chg_chip *chip = g_oneplus_chip;
...
#define ONEPLUS_FV_MV_THRESHOLD 4180
#define ONEPLUS_HT_FV_THRESHOLD 4130
...
static int oneplus_qc_input_current_hanlder(struct oneplus_qc_config *config, void *data) {
	struct oneplus_qc_config_param *param = (struct oneplus_qc_config_param*)data;
	...
	config->state = 0;
	oneplus_qc_set_icl(param->volt <= ONEPLUS_FV_MV_THRESHOLD ? config->qc_hv_icl : config->qc_lv_icl);
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
//		oneplus_qc_set_icl(config->qc_ledon_icl);
...
//		oneplus_qc_set_icl(param->volt <= ONEPLUS_FV_MV_THRESHOLD ? config->qc_hv_icl : config->qc_lv_icl);
	...
	if (param->volt >= ONEPLUS_HT_FV_THRESHOLD && param->temp >= 45) {
		chg_debug("vote disable charge\n");
...
#define USBIN_25MA	25000
static int onepluschg_set_icl(int current_ma)
{
	...
	onepluschg_aicl_enable(false);
	onepluschg_usbin_collapse_irq_enable(false);
	...
	chg_vol = onepluschg_read_adc(USBIN_VOLTAGE);
	if (get_client_vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, BOOST_BACK_VOTER) == 0
		&& get_effective_result(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable) <= USBIN_25MA) {
		chg_debug("use 500 here\n");
		goto aicl_boost_back;
	...
	if (onepluschg_is_suspend_status() && onepluschg_is_usb_present() && onepluschg_is_normal_mode()) {
		i = i - 1;
		goto aicl_suspend;
	}
	chg_vol = onepluschg_read_adc(USBIN_VOLTAGE);
...
aicl_pre_step:
	...
	onepluschg_rerun_aicl();
	onepluschg_usbin_collapse_irq_enable(true);
	goto aicl_return;
...
aicl_suspend:
	...
	onepluschg_check_clear_suspend();
	onepluschg_rerun_aicl();
	onepluschg_usbin_collapse_irq_enable(true);
	goto aicl_return;
...
static int onepluschg_set_fv(int vfloat_mv)
{
	int rc = 0;
	struct oneplus_chg_chip *chip = g_oneplus_chip;
...
static int onepluschg_set_iterm(int term_current)
{
	int rc = 0;
	u8 val_raw = 0;
	struct oneplus_chg_chip *chip = g_oneplus_chip;
...
static int onepluschg_set_rechg_vol(int rechg_vol)
{
	return 0;
}

static int onepluschg_reset_charge(void)
{
	return 0;
}

static int onepluschg_read_full(void)
{
	int rc = 0;
	u8 stat = 0;
	struct oneplus_chg_chip *chip = g_oneplus_chip;

	if (!onepluschg_is_usb_present())
		return 0;
...
static int onepluschg_otg_enable(void)
{
	return 0;
}

static int onepluschg_otg_disable(void)
{
	return 0;
}

static int onepluschg_set_chging_term_disable(void)
{
	int rc;
	struct smb_charger *chg = NULL;
...
static bool onepluschg_check_charger_resume(void)
{
	return true;
}

static bool onepluschg_need_to_check_ibatt(void)
{
	return false;
}

static int onepluschg_get_fcc_step(void)
{
	return 50;
}

static int onepluschg_get_charger_volt(void)
{
	return onepluschg_read_adc(USBIN_VOLTAGE);
}

static int onepluschg_get_ibus(void)
{
	struct smb_charger *chg = NULL;
	union power_supply_propval val;
...
static bool onepluschg_is_usb_present(void)
{
	...
	if ((chip->pmic_spmi.smb5_chip->chg.typec_mode == POWER_SUPPLY_TYPEC_SINK
		...
		&& chip->vbatt_num == 2 ) {
		chg_err("chg->typec_mode = SINK,oneplus_chg_is_usb_present return false!\n");
		rc = false;
		return rc ;
	}
	...
	if (vbus_rising == false && oneplus_warp_get_fastchg_started() == true) {
		if (onepluschg_read_adc(USBIN_VOLTAGE) > 2000) {
			chg_err("USBIN_PLUGIN_RT_STS_BIT low but fastchg started true and chg vol > 2V\n");
			vbus_rising = true;
		}
	}

	if (vbus_rising == false && (oneplus_warp_get_fastchg_started() == true && chip->vbatt_num == 2)) {
...
static int onepluschg_get_battery_voltage(void)
{
	return 3800;	/* Not use anymore */
}

static int onepluschg_get_boot_reason(void)
{
	return 0;
}

static int onepluschg_get_shutdown_soc(void)
{
	u8 val;
	int rc;
	struct smb_charger *chg = NULL;
...
static int onepluschg_backup_soc(int backup_soc)
{
	int rc;
	struct smb_charger *chg = NULL;
...
static int onepluschg_get_aicl_ma(void)
{
	return 0;
}

static int onepluschg_force_tlim_en(bool enable)
{
	return 0;
}

static int onepluschg_set_system_temp_level(int lvl_sel)
{
	return 0;
}

static int onepluschg_set_prop_flash_active(enum skip_reason reason, bool disable)
{
	return 0;
}

static int onepluschg_set_dpdm(int val)
{
	return 0;
}

static int onepluschg_calc_flash_current(void)
{
	return 0;
}

static int onepluschg_get_fv(struct oneplus_chg_chip *chip)
{
...
static int onepluschg_get_charging_current(struct oneplus_chg_chip *chip)
{
...
static int onepluschg_rtc_reset_check(void)
{
	struct rtc_time tm;
 ...
 */
static int onepluschg_get_dyna_aicl_result(void)
{
...
static int onepluschg_get_boot_mode(void)
{
...
#ifdef CONFIG_ONEPLUS_SHORT_HW_CHECK
static bool onepluschg_get_shortc_hw_gpio_status(void)
{
	return true;
}
#else
...
struct oneplus_chg_operations onepluschg_smb5_ops = {
	.dump_registers			= onepluschg_dump_regs,
	.kick_wdt			= onepluschg_kick_wdt,
	.hardware_init			= onepluschg_hw_init,
	.charging_current_write_fast	= onepluschg_set_fcc,
	.set_aicl_point			= onepluschg_set_aicl_point,
	.vbus_voltage_write		= onepluschg_set_vbus,
	.input_current_write		= onepluschg_set_icl,
	.float_voltage_write		= onepluschg_set_fv,
	.term_current_set		= onepluschg_set_iterm,
	.charging_enable		= onepluschg_charge_enble,
	.charging_disable		= onepluschg_charge_disable,
	.get_charging_enable		= onepluschg_get_charge_enable,
	.charger_suspend		= onepluschg_suspend,
	.charger_unsuspend		= onepluschg_unsuspend,
	.set_rechg_vol			= onepluschg_set_rechg_vol,
	.reset_charger			= onepluschg_reset_charge,
	.read_full			= onepluschg_read_full,
	.otg_enable			= onepluschg_otg_enable,
	.otg_disable			= onepluschg_otg_disable,
	.set_charging_term_disable	= onepluschg_set_chging_term_disable,
	.check_charger_resume		= onepluschg_check_charger_resume,
	.get_chargerid_volt		= onepluschg_get_chargerid,
	.set_chargerid_switch_val	= onepluschg_set_chargerid_switch,
	.get_chargerid_switch_val	= onepluschg_get_chargerid_switch,
	.need_to_check_ibatt		= onepluschg_need_to_check_ibatt,
	.get_chg_current_step		= onepluschg_get_fcc_step,
	.get_charger_type		= onepluschg_get_charger_type,
	.get_charger_volt		= onepluschg_get_charger_volt,
	.get_ibus			= onepluschg_get_ibus,
	.check_chrdet_status		= onepluschg_is_usb_present,
	.get_instant_vbatt		= onepluschg_get_battery_voltage,
	.get_boot_mode			= onepluschg_get_boot_mode,
	.get_boot_reason		= onepluschg_get_boot_reason,
	.get_rtc_soc			= onepluschg_get_shutdown_soc,
	.set_rtc_soc			= onepluschg_backup_soc,
	.get_aicl_ma			= onepluschg_get_aicl_ma,
	.rerun_aicl			= onepluschg_rerun_aicl,
	.tlim_en			= onepluschg_force_tlim_en,
	.set_system_temp_level		= onepluschg_set_system_temp_level,
	.otg_pulse_skip_disable		= onepluschg_set_prop_flash_active,
	.set_dp_dm			= onepluschg_set_dpdm,
	.calc_flash_current		= onepluschg_calc_flash_current,
#ifdef CONFIG_ONEPLUS_RTC_DET_SUPPORT
	.check_rtc_reset		= onepluschg_rtc_reset_check,
#endif
#ifdef CONFIG_ONEPLUS_SHORT_C_BATT_CHECK
	.get_dyna_aicl_result		= onepluschg_get_dyna_aicl_result,
#endif
	.set_qc_config		= oneplus_set_qc_config,
	.get_shortc_hw_gpio_status	= onepluschg_get_shortc_hw_gpio_status,
	.get_charger_subtype = oneplus_chg_get_charger_subtype,
};
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	int rc = 0;
	struct oneplus_chg_chip *oneplus_chip;
	struct power_supply *main_psy = NULL;
	union power_supply_propval pval = {0, };

	oneplus_chip = devm_kzalloc(&pdev->dev, sizeof(*oneplus_chip), GFP_KERNEL);
	if (!oneplus_chip)
		return -ENOMEM;

	oneplus_chip->dev = &pdev->dev;
	rc = oneplus_chg_parse_swarp_dt(oneplus_chip);

	if (oneplus_gauge_check_chip_is_null()) {
		chg_err("gauge chip null, will do after bettery init.\n");
		return -EPROBE_DEFER;
	}
	oneplus_chip->chg_ops = &onepluschg_smb5_ops;

	g_oneplus_chip = oneplus_chip;
	...
	oneplus_chip->pmic_spmi.smb5_chip = chip;
	...
	onepluschg_parse_custom_dt(oneplus_chip);
	oneplus_chg_parse_charger_dt(oneplus_chip);
	...
	if (main_psy) {
		pval.intval = 1000 * onepluschg_get_fv(oneplus_chip);
		power_supply_set_property(main_psy,
				POWER_SUPPLY_PROP_VOLTAGE_MAX,
				&pval);
		pval.intval = 1000 * onepluschg_get_charging_current(oneplus_chip);
		power_supply_set_property(main_psy,
				POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
				&pval);
	}
	...
	g_oneplus_chip->authenticate = oneplus_gauge_get_batt_authenticate();
	if(!g_oneplus_chip->authenticate)
		onepluschg_charge_disable();
	oneplus_chg_wake_update_work();
	oneplus_tbatt_power_off_task_init(oneplus_chip);

	INIT_DELAYED_WORK(&onepluschg_debug_work, onepluschg_debug_func);
	schedule_delayed_work(&onepluschg_debug_work, 0);
	...
	push_component_info(FAST_CHARGE, "PMI632+SMB1355", "QualComm");
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (g_oneplus_chip && g_oneplus_chip->enable_shipmode) {
		chg_debug("enter ship mode\n");
		if (g_oneplus_chip->normalchg_gpio.ship_gpio <= 0){
		...
		mutex_lock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		for (i = 0; i < 5; i ++) {
			pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl, g_oneplus_chip->normalchg_gpio.ship_active);
			mdelay(3);
			pinctrl_select_state(g_oneplus_chip->normalchg_gpio.pinctrl, g_oneplus_chip->normalchg_gpio.ship_sleep);
			mdelay(3);
		}
		mutex_unlock(&g_oneplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	}
...
static int onepluschg_get_current_time(unsigned long *now_tm_sec)
{
	struct rtc_time tm;
	struct rtc_device *rtc;
	int rc;
...
static int onepluschg_pm_resume(struct device *dev)
{
	int rc = 0;
	...
	rc = onepluschg_get_current_time(&resume_tm_sec);
	...
	oneplus_chg_soc_update_when_resume(sleep_time);

	return 0;
}

static int onepluschg_pm_suspend(struct device *dev)
{
	if (!g_oneplus_chip)
		return 0;

	if (onepluschg_get_current_time(&suspend_tm_sec)) {
		chg_err("RTC read failed\n");
		suspend_tm_sec = -1;
	}
...
static const struct dev_pm_ops smb5_pm_ops = {
	.resume		= onepluschg_pm_resume,
	.suspend	= onepluschg_pm_suspend,
};
